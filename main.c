...
static void UpdateBuzzer(u8 ,u8 );
/* ---------------------------------------------------------------
...
if(temp > (signed int) g_config.tempSetPoint)
 {
 // Write_str_LCD("BUZZER ON");
  alarmCode |= 0x01; 
  }
if(humidity > (unsigned int)g_config.humiditySetPoint){ alarmCode |= 0x02; }
...
static void UpdateBuzzer(u8 alaram,u8 doorAarm)
{
 if((alarmCode!= 0U)||(doorAlarm!=0U))
 {
   Buzzer_On();
   //Write_str_LCD("BUZZER ON");
 }
...
Buzzer_Off();	  //for a certaion reason
doorAlarm=0U;
return 0U; /* tell main() we sent ThingSpeak data */
}
else
{
...
signed int temp;
unsigned int humidity;		checking by golbal varible 
...
doorAlarm=HandleDoorLogic((u32)SENSOR_SAMPLE_DELAY_MS, &doorOpenMs);

//UpdateBuzzer(alarmCode,doorAlarm);


if(alarmCode!=0U)
{
  Buzzer_On();
  //Write_str_LCD("in mian");
}
else
{
   Buzzer_Off();
...
* -------------------------------------------------------- */
HandleDoorLogic((u32)SENSOR_SAMPLE_DELAY_MS, &doorOpenMs);
